...
public class TCPNetClient implements Runnable {
        ...
        public void run() {
            while (!stop) {
                ...
                try {
                    ...
                    if (!stop) {
                        Log.d(Global.LOG_KEY, buildNetLog("Heartbeat request failed"), e);
    ...
    public void connect() throws Exception {
        ...
        Log.d(Global.LOG_KEY, buildNetLog("ready to connect"));
    ...
    public void close() {
        Log.d(Global.LOG_KEY, buildNetLog("ready to close socket"));
        try {
            stop = true;
            if (socket != null) {
                socket.close();
            ...
            Log.d(Global.LOG_KEY, "TCPNetClient closed");
        } catch (Exception e) {
            Log.d(Global.LOG_KEY, "TCPNetClient close failed", e);
        }
    }
    ...
    private void writeObject(Object obj) throws Exception {
        synchronized (writeObjectLock) {
            Log.d(Global.LOG_KEY, MessageFormat.format("start write object[{0}] to ip:{1}, port:{2}", obj, this.getRemoteAddress(), this.getRemotePort()));
            getOut().reset();
            getOut().writeObject(obj);
            getOut().flush();
            getOut().reset();
            Log.d(Global.LOG_KEY, MessageFormat.format("complete write object[{0}] to ip:{1}, port:{2}", obj, this.getRemoteAddress(), this.getRemotePort()));
        }
    }
    ...
    public TCPResponse sendRequest(TCPRequest request, long resultTimeout) throws Exception {
        ...
        Log.d(Global.LOG_KEY, buildNetLog(MessageFormat.format("TCPNetClient ready to send request[{0}], request id is {1}", request, request.getId())));
        ...
        //发送命令对象到服务端
        try {
            Log.d(Global.LOG_KEY, request.toString());
            writeObject(request);

            Log.d(Global.LOG_KEY, buildNetLog(MessageFormat.format("TCPNetClient has sent: {0}", request)));
        } catch (Exception e) {
            Log.d(Global.LOG_KEY, buildNetLog("TCPNetClient write object error. request : + " + request + "request id is : " + request.getId()), e);
        ...
        //等待结果对象返回
        Log.d(Global.LOG_KEY, buildNetLog(MessageFormat.format("TCPNetClient begin to wait for command result, timeout is {0}", resultTimeout)));
        ...
        synchronized (resultCache) {
            ...
            while (result.response == null) {
                try {
                    ...
                    Log.d(Global.LOG_KEY, buildNetLog("exception"), e);
                }
        ...
        //没有等到结果，表示结果等待超时
        if (isTimeout) {
            Log.d(Global.LOG_KEY, buildNetLog("wait for response timeout"));
    ...
    public void run() {
        while (!stop) {
            Log.d(Global.LOG_KEY, buildNetLog("TCPNetClient wait for object from server"));
            Object obj = null;
            try {
                obj = getIn().readObject();
                Log.d(Global.LOG_KEY, buildNetLog(MessageFormat.format("TCPNetClient received: {0}", obj == null ? "null" : obj.toString())));
                ...
                if (obj == null) {
                    if (!stop) {
                        new Thread("notify_processor_thread") {
                            ...
                            public void run() {
                                for (TCPNetClientListener tl : listeners) {
                                    Log.d(Global.LOG_KEY, "TCPNetClient onClosed Method Start ...");
                                    tl.onClosed();
                                }
                            }
                        }.start();
                    ...
                    Log.d(Global.LOG_KEY, "TCPNetClient receive Obj Is TCPRequest, Request Data is :" + request.getData().toString());
                        ...
                        new Thread() {
                            ...
                            public void run() {
                                ...
                                try {
                                    ...
                                    Log.d(Global.LOG_KEY, buildNetLog("tcp net client send response to server failed"), e);
                                }
                            }
                        }.start();
                    }
                    ...
                    if (((TCPResponse) obj).getData() != null) {
                        Log.d(Global.LOG_KEY, "TCPNetClient receive Obj Is TCPResponse, Response Data is :" + ((TCPResponse) obj).getData().toString());
                    } else {
                        Log.d(Global.LOG_KEY, "TCPNetClient receive Obj Is TCPResponse, Response Data is TCPHeartBeat.");
                    }
                    setRequestResult((TCPResponse) obj);
                } else if (obj instanceof TCPMessage) {
                    final TCPMessage message = (TCPMessage) obj;
                    Log.d(Global.LOG_KEY, "TCPNetClient receive Obj Is TCPMessage, Message Data is :" + message.getData().toString());
                    new Thread() {
                        @Override
                        public void run() {
                            for (TCPNetClientListener tl : listeners) {
                ...
                Log.d(Global.LOG_KEY, buildNetLog("TCPNetClient read object error"), e);
        ...
        Log.d(Global.LOG_KEY, buildNetLog(MessageFormat.format("TCPNetClient receiver loop end[stop:{0}]", stop)));
    ...
    public void setRequestResult(TCPResponse response) {
        synchronized (resultCache) {
            ...
            if (result == null) {
                Log.d(Global.LOG_KEY, buildNetLog(MessageFormat.format("command result already destroyed, can not set command result: {0}", response)));
            } else {
            ...
            Log.d(Global.LOG_KEY, buildNetLog("TCPNetClient done set command result: " + response));
        }
    }
    ...
    private ObjectInputStream getIn() throws IOException {
        synchronized (getInLock) {
            if (objectInputStream == null) {
                ...
                Log.d(Global.LOG_KEY, this + "   ; remoteAddress " + remoteAddress + " ; create ObjectInputStream : " + objectInputStream);
            }
    ...
    private ObjectOutputStream getOut() throws IOException {
        synchronized (getOutLock) {
            if (objectOutputStream == null) {
                ...
                Log.d(Global.LOG_KEY, this + "   ; remoteAddress " + remoteAddress + " ; create ObjectOutputStream : " + objectOutputStream);
            }
        }
