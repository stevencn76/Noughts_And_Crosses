    ...
    private class TCPNetSessionProcessor implements TCPNetSessionContext, Runnable {
        ...
        public TCPResponse sendRequest(TCPRequest request, long resultTimeout) throws Exception {
            ...
            try {
                ...
                Log.d(Global.LOG_KEY, buildNetLog("TCPNetClient write object error"), e);
            ...
            //等待结果对象返回
            Log.d(Global.LOG_KEY, buildNetLog("TCPNetClient begin to wait for command result"));
            ...
            synchronized (resultCache) {
                ...
                while (result.response == null) {
                    try {
                        ...
                        Log.d(Global.LOG_KEY, "exception", e);
                    }
                ...
                try {
                    while (!stop) {
                        ...
                        try {
                            ...
                            Log.d(Global.LOG_KEY, buildNetLog("net server receive command error"), e);
                            continue;
                        }
                        ...
                        if (obj == null) {
                            ...
                            Log.d(Global.LOG_KEY, "TCPNetServer receive Obj Is TCPRequest, Request Data is :" + request.getData().toString());
                            ...
                            if (request.getData() instanceof TCPHeartbeat) {
                                ...
                                new Thread() {
                                    @Override
                                    public void run() {
                                        Log.d(Global.LOG_KEY, buildNetLog(MessageFormat.format("server process TCPRequest start time: {0}", System.currentTimeMillis())));
                                        for (TCPNetServerListener tl : listeners) {
                                            tl.onRequest(TCPNetSessionProcessor.this, request, response);
                                        }
                                        Log.d(Global.LOG_KEY, buildNetLog(MessageFormat.format("server process TCPRequest end, ready to send result start time: {0}", System.currentTimeMillis())));
                                        try {
                                            writeObject(response);
                                        } catch (Exception e) {
                                            Log.d(Global.LOG_KEY, buildNetLog("tcp net server send response to server failed"), e);
                                        }
                                        Log.d(Global.LOG_KEY, buildNetLog(MessageFormat.format("server process TCPRequest end, send result end time: {0}", System.currentTimeMillis())));
                                    }
                                }.start();
                            }
                        } else if (obj instanceof TCPResponse) {
                            if (((TCPResponse) obj).getData() != null) {
                                Log.d(Global.LOG_KEY, "TCPNetServer receive Obj Is TCPResponse, Response Data is :" + ((TCPResponse) obj).getData().toString());
                            } else {
                                Log.d(Global.LOG_KEY, "TCPNetServer receive Obj Is TCPResponse, Response Data is TCPHeartBeat.");
                            }
                            setRequestResult((TCPResponse) obj);
                        } else if (obj instanceof TCPMessage) {
                            final TCPMessage message = (TCPMessage) obj;
                            Log.d(Global.LOG_KEY, "TCPNetServer receive Obj Is TCPMessage, Message Data is :" + message.getData().toString());
                            new Thread() {
                                @Override
                                public void run() {
                                    for (TCPNetServerListener tl : listeners) {
                    ...
                    Log.d(Global.LOG_KEY, buildNetLog("net server exception"), e);
                } catch (Exception e) {
                    Log.d(Global.LOG_KEY, buildNetLog("net server exception"), e);
                }
                Log.i(Global.LOG_KEY, buildNetLog("Net client disconnected!"));
    ...
    public void run() {
        ...
        while (!stop) {
            try {
                ...
                if (!stop)
                    Log.d(Global.LOG_KEY, "NwServer run error", e);
            }
        }
        alive = false;
    }
